...
# ============================================================
# Universal Media Server Installer - Linux
# Installs/configures:
...
case "$DISTRO" in
  ...
  fedora|rhel|rocky|almalinux|centos) PM=dnf ;;
...
install_base(){
  case "$PM" in
    apt)
      ...
      apt-get update
      apt-get install -y ca-certificates curl wget jq openssl python3 unzip git
      ;;
    dnf)
      dnf install -y ca-certificates curl wget jq openssl python3 unzip git
      ;;
    pacman)
      pacman -Sy --noconfirm
      pacman -S --needed --noconfirm curl wget jq openssl python unzip git
      ;;
    zypper)
      zypper --non-interactive refresh
      zypper --non-interactive install curl wget jq openssl python3 unzip git
      ;;
  esac
}
...
MEDIA_DIR="$(readlink -m "${MEDIA_DIR/#\~/$HOME}")"
CONFIG_DIR="$(readlink -m "${CONFIG_DIR/#\~/$HOME}")"
